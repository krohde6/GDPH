...
generate_graph_cumulative_cases <- function(reorganized_cases, reorganized_cases_baseline) {
  n <- length(reorganized_cases[1])
  avg_cases <- as.numeric(avg_cases_per_day(reorganized_cases))
  avg_cases_base <- as.numeric(avg_cases_per_day(reorganized_cases_baseline))
  
  cumulative_cases <- as.numeric(calculate_cumulative_cases(avg_cases))
  cumulative_cases_base <- as.numeric(calculate_cumulative_cases(avg_cases_base))
  
  x <- 1:days
  df <- data.frame(x, cumulative_cases, cumulative_cases_base)
  
  p <- ggplot(data=df, aes(x=x)) +
    geom_line(aes(y = cumulative_cases, colour="Intervention")) +
    geom_point(aes(x=x, y=cumulative_cases, colour="Intervention")) +
    geom_line(aes(y = cumulative_cases_base, colour="No Intervention")) +
    geom_point(aes(x=x, y=cumulative_cases_base, colour="No Intervention")) +
    scale_color_manual("",
            breaks = c("Intervention", "No Intervention"),
            values = c("green", "red")) +
    xlab("Days") + 
...
cases_per_day_multiple_simulations_baseline <- run_simulation_repeated(40, initial_state, testing_baseline, no_testing_baseline, semester)
...
cumulative_cases_graph <- generate_graph_cumulative_cases(reorganized_cases, reorganized_cases_baseline)

print(cases_per_day_graph)
print(cumulative_cases_graph)

#Mean Absolute Percent Error for School-Level Validation
library(MLmetrics)
mape_calculation <- function(total_cases) {
  y_predict <- list()
  y_actual <-list()
  total_population = student_pop + faculty_pop 
  actual_cases = 1
  for (i in 1:40) {
    y_predict[[length(y_predict)+1]] <- total_population - sum(as.numeric(unlist(total_cases[i])))
    y_actual[[length(y_actual)+1]] <- total_population - actual_cases
  }
  #print(y_actual)
  #print(y_pred)
  #percent_error = mean(abs((y_true-y_pred)/y_true)) * 100
  percent_error = MAPE(unlist(y_predict), unlist(y_actual))
  return(percent_error)
}

mape_calculation(total_cases)
```
...
generate_predicted_county_cases <- function(county_data) {
  ...
  for (i in 1:nrow(county_data)) {
    ...
    baseline <- run_simulation_repeated(10, initial_state, testing, no_testing, semester)
    cases <- c()
    for (i in range(1:10)){
