...
RC_dot <- function(gamma, I_a, I_s, TFN_a, TFN_s) {
  result <- gamma * (I_a + I_s + TFN_a + TFN_s)
  return(result)
}

RU_dot <- function(gamma, TISO_ap, TISO_sp, TISO_an, TISO_sn) {
...
replicate_function2 <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    dRC <- RC_dot(gamma, I_a, I_s, TFN_a, TFN_s)
    dRU <- RU_dot(gamma, TISO_ap, TISO_sp, TISO_an, TISO_sn)
    return(list(c(dS, dI_a, dI_s, dTFN_a, dTFN_s, dTISO_ap, dTISO_sp, dTISO_an, dTISO_sn, dRC, dRU)))
    ...
    #dRC <- gamma*(TISO_ap + TISO_sp + TISO_an + TISO_sn)
    #dRU <- gamma*(I_a + I_s + TFN_a + TFN_s)
    #return(list(c(dS, dI_a, dI_s, dTFN_a, dTFN_s, dTISO_ap, dTISO_sp, dTISO_an, dTISO_sn, dRC, dRU)))
  })
}

#initial state
state <- c(S_0 = 0.895, I_a = 0.0025, I_s = 0.0025, TFN_a = 0, TFN_s = 0, TISO_an = 0, TISO_sn = 0, TISO_ap = 0, TISO_sp = 0, RC = 0, RU = 0.10)

for (t in seq(1, 108, by = 1)) {
  ...
  if ((t - 1)%%7 == 0) {
    ...
    parameters <- c(b_a = 0.1, b_s = 0.2, p0 = 0.5, c_a = 0.7, c_s = 0.9, T_pcr = 0, T_rap = 0, gamma = 0.1, FNpcr_a = 0.15, FNpcr_s = 0.15, FNrap_a = 0.3, FNrap_s = 0.3)
  }
  state <- ode(y = state, times = times, func = replicate_function2, parms = parameters)[2,2:12]
  print(state)
}
