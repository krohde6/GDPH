...
# Unable to download Excel file based on how current file path is structured -- Kyle, maybe you can figure out a solution?
...
fulton <- generate_predicted_school_cases(single_county_data)
fulton
```


```{r}
# Cleaner code for validation
library(ggplot2)
...
observed_cases <- populate_predicted_county_cases(keys(counties), county_data$X14.day.cases)
#observed_cases
...
large_error_counties <- validation_table[validation_table$difference >= mean(difference), ]
large_error_counties
...
ggplot(data = large_error_counties, aes(x = large_error_counties$difference)) +
