...
total_cases <- calculate_cases_multiple_simulations(40, results)
...
reorganized_cases <- reorganize_cases_per_day(40, total_cases)

# Get average cases per day
avg_cases_per_day <- function(reorganized_cases) {
  day_avgs <- list()
  for (i in 1:days) {
    day_avgs[[length(day_avgs) + 1]] <- mean(as.numeric(reorganized_cases[[i]]))
  }
  return(day_avgs)
}

# Use this function to graph average cases per day over days in simulation
graph_avgs <- function(cases_per_day) {
  x <- c(1:days)
  y <- avg_cases_per_day(cases_per_day)
  plot(x, y)
  lines(x, y)
}

graph_avgs(reorganized_cases)
```
```
