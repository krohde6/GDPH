...
for (t in semester) {
  ...
  baseline[[length(baseline) + 1]] <- state
}
...
for (t in semester) {
  ...
  state <- ode(y = state, times = times, func = replicate_function, parms = parameters)[2,c(2:12)]
  PCR_100[[length(PCR_100) + 1]] <- state
}
...
for (t in semester) {
  ...
  state <- ode(y = state, times = times, func = replicate_function, parms = parameters)[2,c(2:12)]
  RAP_100[[length(RAP_100) + 1]] <- state
}
