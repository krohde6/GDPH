...
# COLIN: this function can be called with all of the user inputted data AS STRINGS and returns a dictionary for that school
...
# COLIN: here is where we need an integer type variable from the user for student and faculty population (replace the definition of student_population, faculty_population)
...
# COLIN: here is where we need a float type variable from the user for student and faculty population (replace the definition of VR_s, VR_f)
...
# COLIN: here is where I would like a string type variable for which county was selected, can be stored using this convention:
# county <- (string representation of that county)

# COLIN: sorry I'm thinking out loud, alternately you can provide a list with all of the user inputs in one place here using this convention:
# user_inputs <- c(county, days, masking, student_pop, faculty_pop, student_vax, faculty_vax, testing)
# All variables can be strings, but it would be easier on me if numeric values were numeric types
```
...
# CAROLINE: not sure what these variables are defining 
