...
data <- read_excel("/Users/kyle/school/fall2021/senior_design/GDPH/enrollment.xlsx")
attach(data)
...
write_xlsx(data, "/Users/kyle/school/fall2021/senior_design/GDPH/exported_data.xlsx")
```

```{r}
library(hash)

initialize_school_population <- function() {
  grade_population <- hash()
  grade_population["K"] <- 0
  grade_population["1"] <- 0
  grade_population["2"] <- 0
  grade_population["3"] <- 0
  grade_population["4"] <- 0
  grade_population["5"] <- 0
  grade_population["6"] <- 0
  grade_population["7"] <- 0
  grade_population["8"] <- 0
  grade_population["9"] <- 0
  grade_population["10"] <- 0
  grade_population["11"] <- 0
  grade_population["12"] <- 0
  grade_population["staff"] <- 0 # should include faculty
...
districts <- get_unique_districts(data)
districts
...
populate_schools <- function(data) {
  ...
  for (row in data) {
    ...
    districts <- get_unique_districts(data)
    environment_type <- districts[district_number]
    grade_population <- initialize_school_population()
    school <- hash()
    school["population"] <- grade_population
    school["environment_type"] <- environment_type
    school["days"] <- 107
